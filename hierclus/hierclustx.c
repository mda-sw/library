...
void FMatrixCreate(fmatrix_t *mat, int num_rows, int num_cols)
{
	...
	mat->m = (float**)malloc(mat->num_rows * sizeof(float*));

	for (row = 0; row < mat->num_rows; row++)
	{
		mat->m[row] = (float*)malloc(mat->num_cols * sizeof(float));
		memset(mat->m[row], 0, mat->num_cols * sizeof(float));
	}
} // end FMatrixCreate
...
void Dissim(fmatrix_t *diss, const float *data, const float *wts, int num_rows, int num_cols)
{
	int i1, i2, j;
	float tmp, diff_a;
	...
	{	
		for (i2 = 0; i2 < i1; i2++)
		{
			for (j = 0; j < num_cols; j++)
			{
				...
				tmp = (wts[i1] * wts[i2]) / (wts[i1] + wts[i2]) * (float)(diff_a * diff_a);
...
void GetNNs(int *nn, float *nndiss, const fmatrix_t *diss, const int *flag)
{
	int minobs, i1, i2;
	float mindist;
...
void HierClust(int *order, int *ia, int *ib, float *levels, 
			   const float *data, const float *wts, 
			   int *nrows_ptr, int *ncols_ptr)
{
	...
	memset(levels, 0, sizeof(float) * (num_rows-1));
	memset(order, 0, sizeof(int) * num_rows);

	mass = (float*)malloc(sizeof(float) * num_rows);
	memcpy(mass, wts, sizeof(float)* num_rows);
	...
	nndiss = (float*)malloc(sizeof(float) * num_rows);
	memset(nn, 0, sizeof(int) * num_rows);
	memset(nndiss, 0, sizeof(float) * num_rows);

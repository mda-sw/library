C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C                                  
C  Carry out a PRINCIPAL COMPONENTS ANALYSIS (KARHUNEN-LOEVE EXPANSION).  
C                                           
C  To call: CALL PCA(N,M,DATA,METHOD,A1,W1,W2,W3,W4,A2,IERR) 
C           where
C                                
C                                
...
C          = 7: on sample covariance matrix.
C          = 8: on sample correlation matrix.
...
C                                                      
C  Inputs here are N, M, DATA, METHOD (and IERR).
...
C                                 
C  F. Murtagh, ST-ECF/ESA/ESO, Garching/Munich, January 1986. 
C
C  HISTORY
C
...
C  Addition of OPTIONs 7 and 8, and assoc. subrs.      [F.M., Sept. 1991]
C  Remomval of all IPRINT-related subr's. and calls.   [F.M., Feb. 1992]
C                             
C-------------------------------------------------------------------------
        SUBROUTINE PCA(N,M,DATA,METHOD,A,W,FV1,W3,W4,Z,IERR)
...
C
        ...
        IF (METHOD.EQ.7) GOTO 700
        IF (METHOD.EQ.8) GOTO 800
...
C       Now do the PCA.
        GOTO 1000
C
C---    Form sample covariance matrix.
  700   CONTINUE
        CALL PCOVSA(N,M,DATA,W,A)
C       Now do the PCA.
        GOTO 1000
C
C---    Construct sample correlation matrix.
  800   CONTINUE
        CALL PCORSA(N,M,DATA,W,FV1,A)
C       Now do the PCA.
        GOTO 1000
C
...
C++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C                           
C  Determine sample correlations of columns.
C  First determine the means of columns, storing in WORK1.
C                                         
C--------------------------------------------------------
        SUBROUTINE PCORSA(N,M,DATA,WORK1,WORK2,OUT)
        DIMENSION       DATA(N,M), OUT(M,M), WORK1(M), WORK2(M)
        DATA            EPS/1.E-10/
C
        DO 30 J = 1, M
...
C
        DO 70 I = 1, N
           DO 60 J = 1, M
              DATA(I,J) = (DATA(I,J)
     X                    -WORK1(J))/(SQRT(FLOAT(N-1))*WORK2(J))
   60      CONTINUE
   70   CONTINUE
C
C          Finally calc. the cross product of the data matrix.
...
C++++++++++++++++++++++++++++++++++++++++++++++++++++++
C                       
C  Determine sample covariances of columns. 
C  First determine the means of columns, storing in WORK.
C                                
C------------------------------------------------------
        SUBROUTINE PCOVSA(N,M,DATA,WORK,OUT)
        DIMENSION       DATA(N,M), OUT(M,M), WORK(M)
C
        DO 30 J = 1, M
           WORK(J) = 0.0
